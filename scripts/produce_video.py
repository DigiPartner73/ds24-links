...
# SCHRITT 1: ElevenLabs TTS (Fallback: Fish Audio)
print('[1/3] ElevenLabs TTS...')
t0 = time.time()
EL_KEY      = get_config('elevenlabs_api_key')
EL_VOICE_ID = (get_config('elevenlabs_voice_id_DE')
               or get_config('elevenlabs_voice_id')
               or '4RZ84U1b4WCqpu57LvIq')
...
if EL_KEY:
    ...
    print(f'ElevenLabs OK {time.time()-t0:.1f}s | {len(audio_data)//1024}KB')
else:
    FISH_KEY = get_config('fish_audio_api_key')
    if not FISH_KEY:
        fail(rec_id, 'fehler_voice', 'Weder elevenlabs_api_key noch fish_audio_api_key in Config')
    tts = requests.post(
...
project_id = j2v_data.get('project')
if not project_id:
...
for attempt in range(120):
    time.sleep(10)
    poll   = requests.get(f'https://api.json2video.com/v2/movies?project={project_id}',
                          headers={'x-api-key': J2V_KEY}, timeout=15).json()
...
print(f'JSON2Video OK {time.time()-t0:.1f}s | Video: {video_url[:80]}')
